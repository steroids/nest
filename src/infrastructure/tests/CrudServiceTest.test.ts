...
import {ImageSaveDto} from './app/dtos/ImageSaveDto';
import {FileSaveDto} from './app/dtos/FileSaveDto';
import {UserSaveDto} from './app/dtos/UserSaveDto';
import {UserInfoSaveDto} from './app/dtos/UserInfoSaveDto';
import {ArticleSaveDto} from './app/dtos/ArticleSaveDto';
import {TagSaveDto} from './app/dtos/TagSaveDto';

const createPhoto = (app) => app.get(FileService).create(DataMapper.create<FileSaveDto>(FileSaveDto, {
    name: 'file ' + Date.now(),
    images: [
        {
            size: 'thumbnail',
...
describe('CrudServiceTest', () => {
    ...
    it('OneToMany ids', async () => {
        ...
        const additionalImage1 = await app.get(ImageService).create(DataMapper.create<ImageSaveDto>(ImageSaveDto, {
            size: 'thumbnail',
            url: 'https://test.com/image-thumbnail.png',
        }));
        expect(additionalImage1?.id).toBeGreaterThan(0);

        let additionalPhoto = await app.get(FileService).create(DataMapper.create<FileSaveDto>(FileSaveDto, {
            name: 'file ' + Date.now(),
    ...
    it('ManyToMany data', async () => {
        ...
        const user: UserModel = await app.get(UserService).create(DataMapper.create<UserSaveDto>(UserSaveDto, {
            name: 'Test user ' + Date.now(),
            info: DataMapper.create<UserInfoSaveDto>(UserInfoSaveDto, {
        ...
        // Create file and image models with OneToMany::ids relations
        let article = await app.get(ArticleService).create(DataMapper.create<ArticleSaveDto>(ArticleSaveDto, {
            title: 'How to load relations in entities',
            creatorUserId: user.id,
            tags: [
                DataMapper.create<TagSaveDto>(TagSaveDto, {title: 'Tag 1'}),
                DataMapper.create<TagSaveDto>(TagSaveDto, {title: 'Tag 2'}),
            ],
        }));
    ...
    it('Complex test', async () => {
        ...
        let user: UserModel = await app.get(UserService).create(DataMapper.create<UserSaveDto>(UserSaveDto, {
            name: 'Test user ' + Date.now(),
            info: DataMapper.create<UserInfoSaveDto>(UserInfoSaveDto, {
                passport: '0409 123456',
            }),
