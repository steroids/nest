...
- Добавлен параметр `newPermissionsCheck` в `RestApplication` для проверки наличия объявленных в коде прав доступа в БД. Принимает объект с полем `enabled` из env `APP_CHECK_NEW_PERMISSIONS` и с названиями таблицы и колонки прав доступа (по умолчанию `auth_permission` и `name`)
