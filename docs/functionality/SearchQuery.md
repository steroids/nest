...
- `alias` - позволяет задать alias для основной таблицы, который будет использован в запросе. Пример:
    ...
    `orderBy` и `addOrderBy` нормализуют переданный путь поля: добавляют alias основной таблицы, оборачивают части пути в двойные кавычки и преобразуют пути связей в alias связи.

    ```ts
    searchQuery.orderBy('id', 'desc');
    console.log(searchQuery.getOrderBy());
    // { '"model"."id"': 'desc' }

    searchQuery.orderBy('customer.profile.name');
    console.log(searchQuery.getOrderBy());
    // { '"customer_profile"."name"': 'asc' }
    ```

    Для длинных путей связей можно включить короткие алиасы через `useShortAliases`.
    В этом режиме алиас связи формируется как хеш полного пути, что помогает обходить ограничения БД на длину alias и предотвращает коллизии:

    ```ts
    const order = await this.createQuery({useShortAliases: true})
          .with([
