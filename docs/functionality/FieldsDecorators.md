...
- `DateTimeField` - поле для хранения даты и времени. Для поля задан transform, преобразующий значения к формату `yyyy-MM-dd HH:mm:ss`. Если нужно обрезать секунды, передайте `skipSeconds: true`.
