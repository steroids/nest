...
- `BaseField` - декоратор, содержащий базовый набор параметров для остальных декораторов. Параметры:
    - `label` - название поле, отображается в swagger документации
    - `example` - пример значения поля, отображается в swagger документации.
    - `defaultValue` - значение поля по-умолчанию. Передает в TypeORM для задания default значения в базе данных.
    - `required` - является ли поле обязательным для заполнения
    - `nullable` - может значение поля быть равным null
    - `isArray` - является ли поле массивом
    - `noColumn` - поле не будет представлено в БД
    - `transform` - принимает callback, который преобразует значение поля при создании объекта с помощью DataMapper.
