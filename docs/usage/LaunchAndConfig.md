...
размера тела запроса, безопасного завершения работы приложения и подписи кук через `cookieSecret`.
...
- Настройки глобального префикса `/api` и URI versioning с версией по умолчанию `v1`.
- Настройки документации API с помощью Swagger.
- Управления CORS.
- Подключения `cookie-parser` для чтения кук и, при наличии `cookieSecret`, подписанных кук.
